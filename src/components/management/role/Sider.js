...
function Sider(props) {
  ...
  useEffect(() => {
    const redirect = () => {
      if (!id && list.length) {
        const defaultId = service.getValue(list, '0.inqGrpId', null);
        return history.push(`/management/role/${defaultId}/read`);
      }
      return null;
    };
  ...
  const onEvents = events => {
    ...
    if (Object.keys(payload).length) {
      setHandler(false);
      if (method === 'submit') {
        const obj = api.createRoleItem(payload);
  ...
  const getModal = useCallback(() => {
    const matched = values.pages.role.sider.modals.filter(modal => modal.key === 'add').find(modal => modal);
  ...
  return (
    <Card title={values.pages.role.sider.title}>
      <Row type="flex" justify="start" align="stretch" style={{ height: '100%' }}>
        <Col span={24}>
          <Menu theme="light" mode="inline" selectedKeys={[id]}>
            {list.length &&
              list.map((item, idx) => {
                return (
                  <Menu.Item key={service.getValue(item, 'inqGrpId', idx)}>
                    <NavLink to={`/management/role/${service.getValue(item, 'inqGrpId', null)}/read`}>{service.getValue(item, 'inqGrpName', '')}</NavLink>
                  </Menu.Item>
                );
              })}
          </Menu>
        ...
        <Col span={24} style={{ flex: '1 1 auto', alignSelf: 'flex-end', padding: 20 }}>
          {values.pages.role.sider.buttons.map(buttonKey => {
